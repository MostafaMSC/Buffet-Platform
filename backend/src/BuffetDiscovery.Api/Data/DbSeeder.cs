...
public static class DbSeeder
{
    ...
    private static async Task SeedSampleRestaurantsAsync(AppDbContext db)
    {
        ...
        foreach (var s in samples)
        {
            ...
            foreach (var o in s.Offerings)
            {
                var offering = new BuffetOffering
                {
                    ...
                    Weekdays = ParseWeekdays(o.Weekdays),
        ...
        foreach (var offering in offerings)
        {
            ...
            {
                var isActive = offering.Recurrence switch
                {
                    RecurrenceType.Daily => true,
                    RecurrenceType.SpecificWeekdays => offering.Weekdays.HasFlag(ToWeekDay(d.DayOfWeek)),
                    RecurrenceType.RamadanMode => offering.RamadanStartDate.HasValue && offering.RamadanEndDate.HasValue
                        && d >= offering.RamadanStartDate.Value && d <= offering.RamadanEndDate.Value,
                    RecurrenceType.OneOff => offering.OneOffDate == d,
                    _ => false
                };
    ...
    private static WeekDays ParseWeekdays(List<string>? names)
    {
        if (names is null) return WeekDays.None;
        var result = WeekDays.None;
        foreach (var n in names)
        {
            if (Enum.TryParse<WeekDays>(n, true, out var parsed)) result |= parsed;
        }
        return result;
    }
