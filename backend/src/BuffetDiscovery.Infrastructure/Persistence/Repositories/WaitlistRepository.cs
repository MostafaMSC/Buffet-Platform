...
{
    public Task<List<Waitlist>> GetQueueAsync(int? timeSlotId, int offeringId, DateOnly date, CancellationToken ct)
    {
        var query = db.WaitlistEntries.Where(w =>
            w.OfferingId == offeringId && w.Date == date &&
    ...
    public Task<Waitlist?> GetByIdAsync(int id, CancellationToken ct) =>
        db.WaitlistEntries.Include(w => w.Offering).Include(w => w.TimeSlot).FirstOrDefaultAsync(w => w.Id == id, ct);

    public Task<Waitlist?> GetByIdForCustomerAsync(int id, string phone, CancellationToken ct) =>
        db.WaitlistEntries
            .Include(w => w.Offering)!.ThenInclude(o => o!.Restaurant)
    ...
    public async Task<int> GetNextPositionAsync(int? timeSlotId, int offeringId, DateOnly date, CancellationToken ct)
    {
        var query = db.WaitlistEntries.Where(w =>
            w.OfferingId == offeringId && w.Date == date &&
            (w.Status == WaitlistStatus.Waiting || w.Status == WaitlistStatus.Offered));
